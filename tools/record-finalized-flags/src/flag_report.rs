...
/// build, and filter out the flags in the report that metalava has marked as finalized or kept.
pub(crate) fn read_and_filter_flag_report<R: Read>(mut reader: R) -> Result<HashSet<FlagId>> {
    let mut contents = String::new();
    reader.read_to_string(&mut contents)?;
    let iter = contents.lines().filter_map(|s| {
        let parts: Vec<_> = s.split(',').collect();
        if parts.contains(&"finalized") || parts.contains(&"kept") {
