...
pub(crate) fn read_exception_flags<R: Read>(mut reader: R) -> Result<HashSet<FlagId>> {
...
mod tests {
    ...
    #[test]
    fn test() {
        let input = include_bytes!("../tests/non_api_flags_list.txt");
        let flags = read_exception_flags(&input[..]).unwrap();
        assert_eq!(
            flags,
